...
# version 3 with a collapse made at observation
#
######################################
...
def observe(atom, direction, gradient):
    ...
    if direction=="z":
        if atom["z"]["val"]=="?":
            ...
            atom["x"]["obs"] = 0
        #break the x direction and return it to a random state
        atom["x"]["val"] = "?"
    elif direction=="x":
        if atom["x"]["val"]=="?":
            val = r.uniform(-1.0,1.0)
            ...
            atom["z"]["obs"] = 0
        #break the z direction and return it to a random state
        atom["z"]["val"] = "?"
